...
- **Per-browser session isolation, no login.** The app binds to `127.0.0.1` only, so today's real protection is "only your machine can reach it" — but if it's ever pointed at by more than one person (e.g. run on a shared host), there is still no user identity, only projects. `_session_cookie` middleware sets an unguessable `uit_sid` cookie (httponly, 1-year) on first visit; `create_project` stamps `meta["owner"] = sid`, and every project-scoped route (`list/get/delete/upload/mode/model/message/preview/download`) checks `_accessible(pid, sid)` before doing anything, returning **404** (not 403 — never confirm existence) for a mismatched session. Projects created before this feature (no `owner` key in `meta.json`) stay visible to every session — same permissive behavior as before, so nothing already on disk becomes inaccessible; only new projects are isolated. This is **not authentication** — no identity, no password, just "this browser's cookie ≠ that browser's cookie" — so it stops one browser from seeing another's projects, not a determined attacker who clears cookies (they'd just get a fresh, empty session, never someone else's data, since ownership is keyed to the random cookie value, not guessable per-user state).
...
- **Cost counter.** `_CostTracker` prices the `usage` on each stream-json `assistant` event (per-message-id, latest snapshot wins — never double-counted) using `_TIER_PRICING_PER_MTOK`, keyed by the model family inferred from `resolve_model()`; the final `result` event's exact `total_cost_usd` overrides the estimate. Each token snapshot is emitted as a `cost` SSE event. The SPA renders it as a pill in the topbar status (orange while working), quantized to 10-cent steps for all amounts, accumulating across turns in the open session (`costBase`) and resetting on project switch. CLI path only — demo mode emits no cost. `/api/version` also returns `model_family` (`_price_tier(resolve_model()).title()`) so the activity panel says "Claude Opus is working" (or Fable/…) rather than a generic "Claude".
...
- **Model id is Bedrock-aware** (`resolve_model`). On Bedrock the `--model` value MUST be a full inference-profile id (`global.anthropic.claude-opus-4-8`); the bare first-party alias (`claude-opus-4-8`) is rejected with *"model identifier is invalid"*. Resolution order: explicit `UIT_CLAUDE_MODEL` (escape hatch only — deployment config should set the family env vars, not a literal id) → on Bedrock, by family: **fable** → `ANTHROPIC_MODEL` → `ANTHROPIC_DEFAULT_FABLE_MODEL` → `ANTHROPIC_DEFAULT_OPUS_MODEL`; **opus** → `ANTHROPIC_DEFAULT_OPUS_MODEL` → else the family's short alias for the first-party API. Whatever the source, `_ensure_1m` pins the **Opus and Fable families to the `[1m]` (1M-token) variant** — appending the suffix if absent, idempotent, and a no-op for families that don't take it (Sonnet/Haiku) or ids that already carry it. The `[1m]` suffix works on either the short alias or the full inference-profile form. Do **not** hardcode a short alias in the `--model` flag. Default family is **Fable** (`claude-fable-5`, $10/$50 per Mtok — double Opus).
- **Per-project model switch.** A tiny Fable/Opus segmented switch sits left of the cost pill in the topbar (hidden in demo mode and on the landing page). It POSTs `/api/projects/{pid}/model` `{family}`, persisted as `model_family` in `meta.json` (default `"fable"`), and takes effect on the project's next turn — `stream_claude_cli` passes the project's family to `resolve_model`, so the cost tracker and the "Claude Fable/Opus is working" label follow automatically.
