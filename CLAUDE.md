...
- **Five Anthropic skills are hard requirements** (`REQUIRED_SKILLS` in app.py): `docx`, `pdf`, `pptx`, `xlsx`, and `claude-api`. `app.py` symlinks them from `~/.claude/skills/` into each project so Claude reads uploads with the matching skill rather than ad-hoc text extraction (and grounds any Claude/Anthropic API guidance in `claude-api`). It refuses to fill the brief template unless `docx` is installed (`docx_skill_available()` guard). `/api/version` reports `missing_skills`; the SPA shows a startup warning listing any that are absent.
